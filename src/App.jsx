...
export default function PreOfferForm() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    setIsSubmitted(true);
    // Scroll to top so the success overlay looks perfect
    window.scrollTo({ top: 0, behavior: 'smooth' });
  };

  // Shared options for the PDF generator
  const getPdfOptions = () => ({
    margin: 0.5,
    filename: 'Candidate_Information_Form.pdf',
    image: { type: 'jpeg', quality: 0.98 },
    html2canvas: { scale: 2, useCORS: true },
    jsPDF: { unit: 'in', format: 'a4', orientation: 'portrait' }
  });

  // Action: Download PDF Directly
  ...
  const handleShareWhatsApp = async () => {
    ...
    try {
      ...
      const file = new File([pdfBlob], "Candidate_Form.pdf", { type: "application/pdf" });

      // 2. Check if mobile native share supports files
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        await navigator.share({
          files: [file],
          title: 'Candidate Form',
          text: 'Here is my completed pre-offer information form.',
        });
      } else {
        // 3. Desktop Fallback: Download file first, then open WA Web
        const url = URL.createObjectURL(pdfBlob);
        const link = document.createElement('a');
        link.href = url;
        link.download = "Candidate_Form.pdf";
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
        URL.revokeObjectURL(url);

        // Give the browser a split second to start the download before opening the new tab
        setTimeout(() => {
          alert("Since you are on desktop, the PDF has been downloaded automatically.\n\nOpening WhatsApp Web now—just attach the downloaded file to your message!");
          window.open(`https://wa.me/?text=${encodeURIComponent("Hey, attached is my completed Candidate Information Form.")}`, '_blank');
        }, 500);
      }
    } catch (error) {
      console.error("Share aborted or failed:", error);
      // We only alert if it wasn't a user cancellation
      if (error.name !== 'AbortError') {
        alert("Something went wrong with sharing. Please use the Download PDF button instead.");
      }
    }
  };

  return (
    <div className="min-h-screen bg-stone-50 text-slate-900 font-sans relative">
      
      {/* SUCCESS SCREEN OVERLAY - Covers the whole screen but leaves form intact underneath */}
      {isSubmitted && (
        <div className="fixed inset-0 z-50 bg-stone-50 flex items-center justify-center p-6 overflow-y-auto">
          <div className="bg-white border border-gray-200 rounded-2xl p-8 md:p-10 max-w-lg w-full text-center shadow-lg my-auto">
            ...
            <p className="text-gray-500 text-sm mb-8 leading-relaxed">
              Your information is ready. You can download the PDF directly or share it to WhatsApp.
            </p>
            ...
            <div className="flex flex-col gap-3">
              ...
              <button 
                ...
                className="w-full flex items-center justify-center gap-2 bg-white border-2 border-slate-900 hover:bg-slate-50 text-slate-900 py-3 px-6 rounded-xl font-medium transition-colors"
              >
            ...
            <button 
              ...
              className="mt-6 text-sm text-gray-400 hover:text-slate-900 transition-colors"
            >
              ← Back to edit form
            </button>
          </div>
      ...
      <div className="pb-16" ref={formRef}>
        
        {/* HEADER */}
        <div className="bg-slate-900 pt-10 px-6 relative overflow-hidden">
          <div className="max-w-3xl mx-auto relative z-10 pb-8">
            <span className="inline-block bg-amber-500 text-white text-[10px] font-bold tracking-widest uppercase py-1 px-3 rounded-full mb-4">
          ...
          <div className="max-w-3xl mx-auto relative translate-y-[1px]">
        ...
        <div className="max-w-3xl mx-auto px-6 pt-6">
          {/* NOTICE */}
          <div className="bg-amber-50/50 border border-amber-400 rounded-xl p-4 flex gap-3 text-amber-900 text-sm mb-6 shadow-sm">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <section className="bg-white border border-gray-200 rounded-2xl p-6 md:p-8 shadow-sm hover:shadow-md transition-shadow">
              <div className="flex items-center gap-3 mb-6 pb-4 border-b border-gray-100">
                <div className="w-10 h-10 bg-slate-900 text-amber-500 rounded-xl flex items-center justify-center text-xl shrink-0">👤</div>
                <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div className="flex flex-col gap-1.5">
                  <label className="text-xs font-semibold text-slate-900 uppercase tracking-wide">Full Name <span className="text-amber-500">★</span></label>
                  <input type="text" name="full_name" required placeholder="As per CNIC / Passport" className="form-input" />
                </div>
                <div className="flex flex-col gap-1.5">
                  <label className="text-xs font-semibold text-slate-900 uppercase tracking-wide">Position Applied For <span className="text-amber-500">★</span></label>
                ...
                <div className="flex flex-col gap-1.5">
                  <label className="text-xs font-semibold text-slate-900 uppercase tracking-wide">Contact Number <span className="text-amber-500">★</span></label>
                  <input type="tel" name="phone" required placeholder="+92 300 0000000" className="form-input" />
                </div>
                <div className="flex flex-col gap-1.5">
                  <label className="text-xs font-semibold text-slate-900 uppercase tracking-wide">Email Address <span className="text-amber-500">★</span></label>
            ...
            <section className="bg-white border border-gray-200 rounded-2xl p-6 md:p-8 shadow-sm hover:shadow-md transition-shadow">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                <div className="flex flex-col gap-1.5">
                  <label className="text-xs font-semibold text-slate-900 uppercase tracking-wide">Basic Salary <span className="text-amber-500">★</span></label>
            ...
            <section className="bg-white border border-gray-200 rounded-2xl p-6 md:p-8 shadow-sm hover:shadow-md transition-shadow">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-6">
                 <div className="flex flex-col gap-1.5">
                  <label className="text-xs font-semibold text-slate-900 uppercase tracking-wide">Expected Gross Salary <span className="text-amber-500">★</span></label>
                  <div className="relative">
                    <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 text-sm font-medium">₨</span>
            ...
            <section className="bg-white border border-gray-200 rounded-2xl p-6 md:p-8 shadow-sm hover:shadow-md transition-shadow">
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-semibold text-slate-900 uppercase tracking-wide">Candidate Declaration <span className="text-amber-500">★</span></label>
            ...
            {/* SUBMIT AREA */}
            <div className="bg-white border border-gray-200 rounded-2xl p-6 flex flex-col md:flex-row justify-between items-center gap-4 shadow-sm">
